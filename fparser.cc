...
//=========================================================================
namespace {
bool addNewNameData(std::set<NameData>& nameData, std::map<NamePtr, const NameData*>& namePtrs, const NameData& newData) {
    const FuncDefinition* funcDef = findFunction(NamePtr(&(newData.name[0]), unsigned(newData.name.size())));
    ...
    if (dataIter != nameData.end()) {
        if (dataIter->type != newData.type)
            return false;
        namePtrs.erase(NamePtr(&(dataIter->name[0]), unsigned(dataIter->name.size())));
        nameData.erase(dataIter);
    }

    dataIter = nameData.insert(newData).first;
    namePtrs[NamePtr(&(dataIter->name[0]), unsigned(dataIter->name.size()))] = &(*dataIter);
    return true;
}
...
const char* readIdentifier(const char* ptr) {
    ...
    case 5:
        goto loop_5; // E0
    case 3:
        goto loop_3; // E1-EC, EE-EF
    case 4:
        goto loop_4; // C2-DF

    case 1:
        goto loop_1; // F0-F4 XXX F1-F3
    case 6:
        goto loop_6; //       XXX F0
    case 7:
        goto loop_7; //       XXX F4
...
loop:
    ...
    loop_1:
        if (uptr[1] < 0x80 || uptr[1] > 0xBF)
            break;
    len4pos2:
        if (uptr[2] < 0x80 || uptr[2] > 0xBF)
            break;
        if (uptr[3] < 0x80 || uptr[3] > 0xBF)
            break;
        uptr += 4;
        goto loop;
    case 7: // F4:
...
FunctionParser::Data::Data(const Data& rhs)
    ...
    for (std::set<NameData>::const_iterator iter = nameData.begin(); iter != nameData.end(); ++iter) {
        namePtrs[NamePtr(&(iter->name[0]), unsigned(iter->name.size()))] = &(*iter);
    }
}
...
FunctionParser::FunctionParser()
    : delimiterChar(0),
      parseErrorType(NO_FUNCTION_PARSED_YET),
      evalErrorType(0),
      data(new Data),
...
bool FunctionParser::AddConstant(const std::string& name, double value) {
    if (!containsOnlyValidNameChars(name))
        return false;
...
bool FunctionParser::AddFunction(const std::string& name, FunctionPtr ptr, unsigned paramsAmount) {
    if (!containsOnlyValidNameChars(name))
        return false;
...
bool FunctionParser::RemoveIdentifier(const std::string& name) {
    ...
    if (dataIter != data->nameData.end()) {
        data->namePtrs.erase(NamePtr(&(dataIter->name[0]), unsigned(dataIter->name.size())));
...
const char* const ParseErrorMessage[] = {
    ...
    "Not enough memory", // 5
    "An unexpected error occurred. Please make a full bug report to the author", // 6
    "Syntax error in parameter 'Vars' given to FunctionParser::Parse()", // 7
...
bool FunctionParser::ParseVariables(const std::string& inputVarString) {
    if (data->variablesString == inputVarString)
        return true;
    ...
    while (beginPtr < finalPtr) {
        ...
        std::map<NamePtr, const NameData*>::iterator nameIter = data->namePtrs.find(namePtr);
...
// -------------------------
int FunctionParser::Parse(const char* Function, const std::string& Vars, bool useDegrees) {
    CopyOnWrite();
...
int FunctionParser::ParseFunction(const char* function, bool useDegrees) {
    ...
    data->ByteCode.clear();
    data->ByteCode.reserve(128);
    data->Immed.clear();
    data->Immed.reserve(128);
...
//=========================================================================
inline const char* FunctionParser::SetErrorType(ParseErrorType t, const char* pos) {
    parseErrorType = t;
    errorLocation = pos;
    return 0;
}
...
bool IsEligibleIntPowiExponent(int int_exponent) {
    ...
    return (abs_int_exponent >= 1) && (abs_int_exponent <= 46 ||
...
inline void FunctionParser::AddFunctionOpcode(unsigned opcode) {
    if (data->ByteCode.back() == cImmed) {
        ...
        case cAbs:
            ...
            return;
        case cAcos:
            if (data->Immed.back() < -1 || data->Immed.back() > 1)
                break;
        ...
        case cPow: {
            ...
            if (before_immed_opcode == cExp || before_immed_opcode == cExp2 || before_immed_opcode == cPow) {
...
#undef eliminate_redundant_sequence
    ...
    case cExp:
        ...
        if (data->ByteCode.back() == cAdd && data->ByteCode[data->ByteCode.size() - 2] == cImmed) {
...
inline void FunctionParser::AddFunctionOpcode_CheckDegreesConversion(unsigned opcode) {
    ...
    switch (opcode) {
    case cMin:
        if (data->ByteCode.back() == cImmed && data->ByteCode[data->ByteCode.size() - 2] == cImmed) {
            data->Immed[data->Immed.size() - 2] = Min(data->Immed[data->Immed.size() - 2], data->Immed.back());
            data->ByteCode.pop_back();
            data->Immed.pop_back();
            goto skip_op;
        }
    ...
    case cMax:
        if (data->ByteCode.back() == cImmed && data->ByteCode[data->ByteCode.size() - 2] == cImmed) {
            data->Immed[data->Immed.size() - 2] = Max(data->Immed[data->Immed.size() - 2], data->Immed.back());
            data->ByteCode.pop_back();
            data->Immed.pop_back();
            goto skip_op;
        }
    ...
    case cAtan2:
        if (data->ByteCode.back() == cImmed && data->ByteCode[data->ByteCode.size() - 2] == cImmed) {
            data->Immed[data->Immed.size() - 2] = atan2(data->Immed[data->Immed.size() - 2], data->Immed.back());
            data->ByteCode.pop_back();
            data->Immed.pop_back();
            goto skip_op;
        }
...
inline void FunctionParser::AddBinaryOperationByConst() {
    ...
    if (!Operation::valid_rvalue(data->Immed.back())) {
        ...
        data->Immed.pop_back();
        data->ByteCode.pop_back();
    } else if (data->ByteCode[data->ByteCode.size() - 2] == Operation::opcode && data->ByteCode[data->ByteCode.size() - 3] == cImmed) {
     ...
     * x-2+4 = x+(4-2)
     */
    else if (data->ByteCode[data->ByteCode.size() - 2] == Operation::opposite && data->ByteCode[data->ByteCode.size() - 3] == cImmed && Operation::valid_opposite_rvalue(data->Immed[data->Immed.size() - 2])) {
...
inline FunctionParser::ParseErrorType noCommaError(char c) {
    return c == ')' ?
               FunctionParser::ILL_PARAMS_AMOUNT :
...
inline FunctionParser::ParseErrorType noParenthError(char c) {
    return c == ',' ?
               FunctionParser::ILL_PARAMS_AMOUNT :
               FunctionParser::MISSING_PARENTH;
}
} // namespace

const char* FunctionParser::CompileIf(const char* function) {
    if (*function != '(')
        return SetErrorType(EXPECT_PARENTH_FUNC, function);
...
const char* FunctionParser::CompileFunctionParams(const char* function, unsigned requiredParams) {
    if (*function != '(')
        return SetErrorType(EXPECT_PARENTH_FUNC, function);
...
const char* FunctionParser::CompileElement(const char* function) {
    ...
    if (endPtr != function) { // Function, variable or constant
        ...
        if (funcDef && funcDef->enabled) { // is function
            ...
            const unsigned requiredParams = funcDef->opcode == cEval ?
        ...
        std::map<NamePtr, unsigned>::iterator varIter = data->variableRefs.find(name);
        ...
        if (nameIter != data->namePtrs.end()) {
            ...
            case NameData::FUNC_PTR:
                function = CompileFunctionParams(endPtr, data->FuncPtrs[nameData->index].params);
            ...
            case NameData::PARSER_PTR:
                function = CompileFunctionParams(endPtr, data->FuncParsers[nameData->index].params);
    ...
    if (*function == '^') {
        ...
        if (data->ByteCode.back() == cImmed) {
            ...
            if (base_immed > 0.0) {
                ...
                // uh-oh, we've got e.g. (-5)^x, and we already deleted -5 from the stack
...
inline const char* FunctionParser::CompileMult(const char* function) {
    ...
    while ((op = *function) == '*' || op == '/' || op == '%') {
        ...
        bool is_unary = false;
        if (op != '%' && data->ByteCode.back() == cImmed && data->Immed.back() == 1.0) {
            is_unary = true;
            data->Immed.pop_back();
            data->ByteCode.pop_back();
        }
        ...
        default:
            ...
            case '/':
                if (is_unary)
                     ...
                     */
                    if (data->ByteCode.back() == cExp && data->ByteCode[data->ByteCode.size() - 2] == cMul && data->ByteCode[data->ByteCode.size() - 3] == cImmed) {
                        data->Immed.back() = -data->Immed.back();
                        data->ByteCode.push_back(cMul);
                    } else
            ...
            case '*':
                if (is_unary) {
                    ...
                    if (data->ByteCode.back() == cPow && data->ByteCode[data->ByteCode.size() - 2] == cImmed && data->Immed.back() < 0) {
                        data->Immed.back() = -data->Immed.back();
                        data->ByteCode.push_back(cDiv);
                    } else
                        data->ByteCode.push_back(cMul);
...
inline const char* FunctionParser::CompileAddition(const char* function) {
    ...
    while ((op = *function) == '+' || op == '-') {
        ...
        bool is_unary = false;
        if (data->ByteCode.back() == cImmed && data->Immed.back() == 0.0) {
            is_unary = true;
            data->Immed.pop_back();
            data->ByteCode.pop_back();
        }
...
double FunctionParser::Eval(const double* Vars) {
    ...
    for (IP = 0; IP < ByteCodeSize; ++IP) {
        ...
        case cPow:
            ...
            break;
        case cRPow:
            Stack[SP - 1] = pow(Stack[SP], Stack[SP - 1]);
            --SP;
            break;
        ...
        case cFCall: {
            ...
            double retVal = data->FuncPtrs[index].funcPtr(&Stack[SP - params + 1]);
            SP -= int(params) - 1;
            Stack[SP] = retVal;
            break;
        }
        ...
        case cPCall: {
            ...
            double retVal = data->FuncParsers[index].parserPtr->Eval(&Stack[SP - params + 1]);
            SP -= int(params) - 1;
            Stack[SP] = retVal;
            const int error = data->FuncParsers[index].parserPtr->EvalError();
            if (error) {
                evalErrorType = error;
                return 0;
            }
...
int deduceVariables(FunctionParser& fParser,
    ...
    while (true) {
        destVarString.clear();
        for (StrSet::iterator iter = varNames.begin(); iter != varNames.end(); ++iter) {
            if (iter != varNames.begin())
                destVarString += ",";
            destVarString += *iter;
        }

        const int index = fParser.Parse(funcStr, destVarString, useDegrees);
        if (index < 0)
            break;
        if (index == oldIndex)
            return index;
...
int FunctionParser::ParseAndDeduceVariables(const std::string& function, int* amountOfVariablesFound, bool useDegrees) {
    std::string varString;
    return deduceVariables(*this, function.c_str(), varString, amountOfVariablesFound, 0, useDegrees);
}

int FunctionParser::ParseAndDeduceVariables(const std::string& function, std::string& resultVarString, int* amountOfVariablesFound, bool useDegrees) {
    std::string varString;
    const int index = deduceVariables(*this, function.c_str(), varString, amountOfVariablesFound, 0, useDegrees);
    if (index < 0)
        resultVarString = varString;
    return index;
}

int FunctionParser::ParseAndDeduceVariables(const std::string& function, std::vector<std::string>& resultVars, bool useDegrees) {
    std::string varString;
    std::vector<std::string> vars;
    const int index = deduceVariables(*this, function.c_str(), varString, 0, &vars, useDegrees);
    if (index < 0)
        resultVars.swap(vars);
    return index;
}
...
inline void printHex(std::ostream& dest, unsigned n) {
    ...
    dest.width(4);
    dest.fill('0');
    std::hex(dest); //uppercase(dest);
    dest << n;
    dest.flags(flags);
}

void padLine(std::ostringstream& dest, unsigned destLength) {
    for (size_t currentLength = dest.str().length(); currentLength < destLength; ++currentLength) {
        dest << ' ';
    }
}
...
std::string findVariableName(const VariablesMap& varMap, unsigned index) {
    for (VariablesMap::const_iterator iter = varMap.begin(); iter != varMap.end(); ++iter) {
        if (iter->second == index)
            return std::string(iter->first.name, iter->first.name + iter->first.nameLength);
    }
    return "?";
}
...
double ParsePowiMuli(
    ...
    while (IP < limit) {
        ...
        if (ByteCode[IP] == cFetch) {
            unsigned index = ByteCode[++IP];
            if (index < factor_stack_base || size_t(index - factor_stack_base) >= stack.size()) {
...
double ParsePowiSequence(const std::vector<unsigned>& ByteCode, unsigned& IP, unsigned limit, size_t factor_stack_base) {
    FactorStack stack;
    stack.push_back(1.0);
    return ParsePowiMuli(iseq_powi, ByteCode, IP, limit, factor_stack_base, stack);
}

double ParseMuliSequence(const std::vector<unsigned>& ByteCode, unsigned& IP, unsigned limit, size_t factor_stack_base) {
    FactorStack stack;
    stack.push_back(1.0);
...
void FunctionParser::PrintByteCode(std::ostream& dest, bool showExpression) const {
    ...
    after_powi_or_muli:;
        ...
        if (showExpression && !if_stack.empty() &&
            ...
            if (showExpression && (opcode == cSqr || opcode == cDup || opcode == cInv || opcode == cSqrt || opcode == cRSqrt
#ifdef FP_SUPPORT_OPTIMIZER
                                   || opcode == cFetch
#endif
                                   )) {
                ...
                double exponent = ParsePowiSequence(ByteCode, changed_ip,
                                                    if_stack.empty() ? (unsigned)ByteCode.size() : if_stack.back().endif_location,
                                                    stack.size() - 1);
                ...
                for (const unsigned first_ip = IP; IP < changed_ip; ++IP) {
                    ...
                    if (IP >= explanation_before) {
                        ...
                        output << explanation_prefix << '[' << (stack.size() - 1) << ']';
        ...
        not_powi_or_muli:;
            ...
            case cFCall: {
                ...
                std::set<NameData>::const_iterator iter = data->nameData.begin();
                while (iter->type != NameData::FUNC_PTR || iter->index != index)
                    ++iter;
            ...
            case cPCall: {
                ...
                std::set<NameData>::const_iterator iter = data->nameData.begin();
                while (iter->type != NameData::PARSER_PTR || iter->index != index)
                    ++iter;
            ...
            default:
                if (OPCODE(opcode) < VarBegin) {
                    ...
                    if (showExpression) {
                        stack.push_back(std::make_pair(0, (findVariableName(data->variableRefs, opcode))));
                    }
        ...
        if (showExpression) {
            ...
            if ((opcode == cIf && params != 3) ||
                ...
                output << '[' << (stack.size() - 1) << ']' << stack.back().second;
        }
